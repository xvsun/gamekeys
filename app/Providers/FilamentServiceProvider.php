    ...
    public function boot()
    {
        Filament::serving(function () {
            \Filament\Support\Components\ViewComponent::macro('localize', function (?string $translatorPrefix = '', ?bool $label = true, ?bool $helper = true, ?bool $hint = true, ?bool $datetime = false, ?array $labelAttributes = [], ?array $helperAttributes = [], ?array $hintAttributes = []) {
                ...
                /*if ($datetime) {
                    if ($this instanceof \Filament\Forms\Components\TimePicker) {
                        $this->displayFormat(__('app.preferences.time.'.Auth::user()->preference->time_format.'.carbon'));
                    } elseif ($this instanceof \Filament\Forms\Components\DatePicker) {
                        $this->displayFormat(__('app.preferences.date.'.Auth::user()->preference->date_format.'.carbon'));
                    } elseif ($this instanceof \Filament\Forms\Components\DateTimePicker) {
                        $this->displayFormat(__('app.preferences.datetime.'.Auth::user()->preference->datetime_format.'.carbon'));
                    }
                }*/
